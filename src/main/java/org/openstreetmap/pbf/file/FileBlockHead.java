...
public class FileBlockHead extends FileBlockReference {
     ...
     * skipped or read
     */
    static public FileBlockHead readHead(DataInputStream datinput) throws IOException {
        ...
        if (header.getDatasize() > MAX_BODY_SIZE) {
          throw new FileFormatException("Unexpectedly long body "+header.getDatasize()+ " bytes. Possibly corrupt file.");
        }
        
        fileblock.input = datinput;
    ...
    public ByteBuffer readBytes(FileChannel input) throws IOException
    {
    	ByteBuffer dst  = ByteBuffer.allocateDirect(getDatasize());
    	input.read(dst);
    	return dst;
    	
    }
    
    public byte[] readBytes(DataInputStream input) throws IOException
    {
    	byte buf[] = new byte[getDatasize()];
        input.readFully(buf);
        return buf;
